...
// 参加者を取得する
func retrievePlayer(ctx context.Context, db dbOrTx, tenantID int64, id string) (*PlayerRow, error) {
	var p PlayerRow
	if err := db.GetContext(ctx, &p, "SELECT * FROM player WHERE id = ? AND tenant_id = ?", id, tenantID); err != nil {
...
// 参加者を認可する
// 参加者向けAPIで呼ばれる
func authorizePlayer(ctx context.Context, db dbOrTx, tenantID int64, id string) error {
	player, err := retrievePlayer(ctx, db, tenantID, id)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
// 大会を取得する
func retrieveCompetition(ctx context.Context, db dbOrTx, tenantID int64, id string) (*CompetitionRow, error) {
	var c CompetitionRow
	if err := db.GetContext(ctx, &c, "SELECT * FROM competition WHERE id = ? AND tenant_id = ?", id, tenantID); err != nil {
...
// 大会ごとの課金レポートを計算する
func billingReportByCompetition(ctx context.Context, db dbOrTx, tenantID int64, competitonID string) (*BillingReport, error) {
	comp, err := retrieveCompetition(ctx, db, tenantID, competitonID)
	if err != nil {
...
func tenantsBillingHandler(c echo.Context) error {
	...
	for _, t := range ts {
		...
		err := func(t TenantRow) error {
			...
			for _, comp := range cs {
				report, err := billingReportByCompetition(ctx, db, t.ID, comp.ID)
				if err != nil {
...
func playerDisqualifiedHandler(c echo.Context) error {
	...
	p, err := retrievePlayer(ctx, db, v.tenantID, playerID)
	if err != nil {
		// 存在しないプレイヤー
		if errors.Is(err, sql.ErrNoRows) {
...
func competitionScoreHandler(c echo.Context) error {
	...
	for {
		...
		if _, err := retrievePlayer(ctx, db, v.tenantID, playerID); err != nil {
			// 存在しない参加者が含まれている
			if errors.Is(err, sql.ErrNoRows) {
				return echo.NewHTTPError(
					http.StatusBadRequest,
...
func billingHandler(c echo.Context) error {
	...
	for _, comp := range cs {
		report, err := billingReportByCompetition(ctx, db, v.tenantID, comp.ID)
		if err != nil {
...
func playerHandler(c echo.Context) error {
	...
	if err := authorizePlayer(ctx, db, v.tenantID, v.playerID); err != nil {
		return err
	}
	...
	psds := make([]PlayerScoreDetail, 0)
	ids := make([]string, 0)
	for _, c := range cs {
		found := false
		for _, id := range ids {
			if id == c.CompetitionID {
				found = true
			}
		}
		if found {
		...
		ids = append(ids, c.CompetitionID)
	}
...
func competitionRankingHandler(c echo.Context) error {
	...
	// 大会の存在確認
	competition, err := retrieveCompetition(ctx, db, v.tenantID, competitionID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return echo.NewHTTPError(http.StatusNotFound, "competition not found")
		}
	...
	now := time.Now().Unix()
	var tenant TenantRow
	if err := db.GetContext(ctx, &tenant, "SELECT * FROM tenant WHERE id = ?", v.tenantID); err != nil {
...
func meHandler(c echo.Context) error {
	...
	//}
	ctx := context.Background()
	p, err := retrievePlayer(ctx, db, v.tenantID, v.playerID)
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return c.JSON(http.StatusOK, SuccessResult{
				Status: true,
